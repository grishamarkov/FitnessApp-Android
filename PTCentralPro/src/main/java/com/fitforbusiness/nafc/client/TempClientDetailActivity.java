...
public class TempClientDetailActivity extends ActionBarActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            client_id = this.getIntent().getStringExtra("_id");
        } catch (Exception e) {
            e.printStackTrace();
        }

        Bundle bundle = new Bundle();
        bundle.putString(Utils.ARG_GROUP_OR_CLIENT_ID, client_id);
        bundle.putInt(Utils.ARG_GROUP_OR_CLIENT, Utils.FLAG_CLIENT);

        mTabHost = (FragmentTabHost) findViewById(android.R.id.tabhost);
        mTabHost.setup(this, getSupportFragmentManager(), R.id.realtabcontent);

        mTabHost.addTab(mTabHost.newTabSpec("My Client")
                        .setIndicator("", getResources().getDrawable(R.drawable.tab_client_group_detail_bg_selector)),
                ClientDrawerFragment.class, bundle
        );
        ...
        switch (item.getItemId()) {
            case R.id.menuMembership:
                startActivityForResult(new Intent(this, AddMemberShipActivity.class)
                        ...
                        .putExtra(Utils.ARG_GROUP_OR_CLIENT_ID, client_id), Utils.MEMBERSHIP);
                break;
            case R.id.menuNewSession:
                startActivityForResult(new Intent(this, AddSessionActivity.class)
                        .putExtra(Utils.ARG_GROUP_OR_CLIENT, Utils.FLAG_CLIENT)
                        .putExtra(Utils.ARG_GROUP_OR_CLIENT_ID, client_id), Utils.SESSION);
                break;
            case R.id.menuNewPreWorkOutAssessment:
                startActivityForResult(new Intent(this, AssessmentFormList.class)
                        .putExtra(Utils.ARG_GROUP_OR_CLIENT, Utils.FLAG_CLIENT)
                        .putExtra(Utils.ARG_GROUP_OR_CLIENT_ID, client_id), Utils.PRE_WORKOUT_ASSESSMENT);
                break;
